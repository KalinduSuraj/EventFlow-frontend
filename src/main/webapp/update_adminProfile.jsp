...
<head>
    ...
    <script>
        async function updateUserProfile() {
            ...
            const apiUrl = "{{host}}/api/portfolios/2"; // Updated to new API endpoint
            ...
            try {
                ...
                if (!response.ok) {
                    throw new Error(`Failed to update user profile. HTTP Code: ${response.status}`);
                }

                const result = await response.json();
                document.getElementById('successMessage').innerText = "Profile updated successfully!";
            } catch (error) {
                document.getElementById('errorMessage').innerText = `Error updating user profile: ${error.message}`;
            }
        }
    </script>
</head>
...
<div class="bg-white p-6 rounded shadow-md w-full max-w-lg">
    <h1 class="text-2xl font-bold mb-4 text-center">Update User Profile</h1>

    <p id="errorMessage" class="text-red-500 mb-4"></p>
    <p id="successMessage" class="text-green-500 mb-4"></p>

    <form id="profileForm" method="POST" class="space-y-4" onsubmit="event.preventDefault(); updateUserProfile();">
        ...
        <div>
            <button type="submit" class="w-full bg-indigo-600 text-white py-2 px-4 rounded-md hover:bg-indigo-700">Update Profile</button>
        </div>
    </form>
</div>
</body>
