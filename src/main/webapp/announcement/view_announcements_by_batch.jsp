...
<head>
  ...
  <title>Assigned Announcements</title>
...
<div class="container mx-auto p-6">
  <div class="mb-6 py-5">
    <h1 class="text-3xl font-semibold text-gray-800 text-center">Assigned Announcements by Batch</h1>
  </div>
  ...
  <table class="min-w-full table-auto bg-white border-collapse shadow-lg rounded-lg pb-3">
    ...
    <tr>
      <th class="px-6 py-4 text-left">Announcement ID</th>
    ...
    <%
      try {
        List<AnnouncementDTO> assignedAnnouncements = (List<AnnouncementDTO>) request.getAttribute("assignedAnnouncements");

        if (assignedAnnouncements != null && !assignedAnnouncements.isEmpty()) {
          for (AnnouncementDTO announcement : assignedAnnouncements) {
    %>
    ...
    %>
    <tr>
      <td colspan="6" class="px-6 py-4 text-center text-gray-600">No announcements assigned to this batch.</td>
    </tr>
    <%
      }
    } catch (Exception e) {
